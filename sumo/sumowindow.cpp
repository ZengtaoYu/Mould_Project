#include "sumowindow.h"
#include "ui_sumowindow.h"
...
#include "sumo/sumoedit.h"
#include "sumo/sumokeep.h"

SumoWindow::SumoWindow(QVariant authority, QVariant name, QWidget *mainWin, QWidget *parent)
    : ui(new Ui::SumoWindow), mainWindow(mainWin), authvalue_(authority), namevalue_(name) {
    Q_UNUSED(parent);
    ui->setupUi(this);
    ...
    m_Header << QString("定期保养") << QString("特定易损件") << QString("大保养");
    ...
    connect(timer, &QTimer::timeout, this, &SumoWindow::refresh_information);
    timer->start(1000);
    refresh_data();
}

SumoWindow::~SumoWindow() {
    delete ui;
}

void SumoWindow::closeEvent(QCloseEvent *event) {
    QStringList windowNames = {
        tr("成型模具增加"), tr("成型模具产量输入"), tr("异常维修"),      tr("关单保养"), tr("转单换线"),
        tr("定期保养"),     tr("大保养"),           tr("呆滞6个月以上"), tr("其它"),     tr("成型模具参数修改"),
...
void SumoWindow::refresh_data() {
    ...
    if(!config.value("SELECTIONCONFIG/Mold").toString().isEmpty()) {
        ...
        if(!matches.isEmpty()) {
            ...
            ui->ModleList->selectionModel()->select(targetIndex,
            ...
            if(!query.exec()) {
                ...
                if(query.next()) {
                    ui->RuleView->setItem(0, 0, new QTableWidgetItem(query.value("定期保养模次数").toString()));
                    ui->RuleView->setItem(0, 1, new QTableWidgetItem(query.value("特定易损件管制模次数").toString()));
                    ui->RuleView->setItem(0, 2, new QTableWidgetItem(query.value("大保养模次数").toString()));
                    ...
                    ui->RuleView->setItem(1, 0, new QTableWidgetItem(query.value("定期保养模累计数").toString()));
                    ui->RuleView->setItem(1, 1, new QTableWidgetItem(query.value("特定易损件管制模累计数").toString()));
                    ui->RuleView->setItem(1, 2, new QTableWidgetItem(query.value("大保养模累计数").toString()));
                    ...
                    if(ui->RuleView->item(0, 0)->text().toDouble() * 0.9 < ui->RuleView->item(1, 0)->text().toDouble()) {
                        ui->RuleView->item(1, 0)->setBackground(QColor("red"));
                    }
                    if(ui->RuleView->item(0, 1)->text().toDouble() * 0.9 < ui->RuleView->item(1, 1)->text().toDouble()) {
                        ui->RuleView->item(1, 1)->setBackground(QColor("red"));
                    }
                    if(ui->RuleView->item(0, 2)->text().toDouble() * 0.9 < ui->RuleView->item(1, 2)->text().toDouble()) {
...
void SumoWindow::refresh_information() {
    if(handleWorks) {
        return;
    }
    refresh_time++;
...
void SumoWindow::on_RefreshButton_clicked() {
    refresh_data();
}
...
void SumoWindow::onTrayIconActivated(QSystemTrayIcon::ActivationReason reason) {
    if(reason == QSystemTrayIcon::DoubleClick) {
        SumoWindow::showNormal();
        SumoWindow::activateWindow();
        if(message) {
            handleMessage = new HandleMessage(curr_auth, curr_name, this);
            handleMessage->setWindowTitle("工单信息管理");
            m_windowMap.insert(tr("工单信息管理"), handleMessage);
...
void SumoWindow::on_ModleList_clicked(const QModelIndex &index) {
...
void SumoWindow::on_MoldID_returnPressed() {
    QSqlQuery query;
...
void SumoWindow::on_MoldID_textChanged(const QString &arg1) {
    if(arg1.isEmpty()) {
        QSqlQuery query;
        QSqlQueryModel *model;
        model = new QSqlQueryModel(ui->ModleList);
...
void SumoWindow::on_EditButton_clicked() {
    if(id_value.isEmpty()) {
        QMessageBox::warning(this, "模具参数修改", "请选择模具", QMessageBox::Cancel);
        return;
    }
    sumoEdit = new SumoEdit(id_value, this);
    connect(sumoEdit, SIGNAL(mysignal()), this, SLOT(refresh_data()));
    sumoEdit->setWindowTitle("成型模具参数修改");
    m_windowMap.insert(tr("成型模具参数修改"), sumoEdit);
    sumoEdit->show();
}

void SumoWindow::on_EnterButton_clicked() {
    if(id_value.isEmpty()) {
        QMessageBox::warning(this, "录入产量", "请选择模具", QMessageBox::Cancel);
        return;
    }
    sumoEnter = new SumoEnter(id_value);
    connect(sumoEnter, SIGNAL(mysignal()), this, SLOT(refresh_data()));
    sumoEnter->setWindowTitle("成型模具产量输入");
    m_windowMap.insert(tr("成型模具产量输入"), sumoEnter);
    sumoEnter->show();
}

void SumoWindow::on_FindButton_clicked() {
...
void SumoWindow::on_DeleteButton_clicked() {
    QSqlQuery query;
    int machine_num = 0;
    QString mold_id = ui->MoldID->text();
...
void SumoWindow::on_ModleList_doubleClicked(const QModelIndex &index) {
    Q_UNUSED(index);
    if(id_value.isEmpty()) {
        QMessageBox::warning(this, "录入产量", "请选择模具", QMessageBox::Cancel);
        return;
...
void SumoWindow::on_AddButton_clicked() {
    sumoAdd = new SumoAdd(this);
    connect(sumoAdd, SIGNAL(mysignal()), this, SLOT(refresh_data()));
    sumoAdd->setWindowTitle("成型模具增加");
    m_windowMap.insert(tr("成型模具增加"), sumoAdd);
    sumoAdd->show();
}

void SumoWindow::on_AIButton_clicked() {
...
void SumoWindow::on_BomButton_clicked() {
...
void SumoWindow::on_CheckButton_clicked() {
    bool ok;
    QStringList items;
    ...
    if(ok && !selected.isEmpty()) {
        ...
        QSettings config("userconfig.ini", QSettings::IniFormat);
        QXlsx::Document xlsx(":/ducument/check_cxkeep.xlsx");
        QString sql =
            QString("select distinct 模具编号,产品料号 from keep_message where (状态描述 = '定期保养' or 状态描述 = '大保养') and 日期 like '%1%';").arg(
            selected);
        if(query.exec(sql)) {
            while(query.next()) {
                QString mold_id = query.value("模具编号").toString();
                xlsx.write(row, 1, query.value("模具编号").toString(), Song12C);
                xlsx.write(row, 2, query.value("产品料号").toString(), Song12C);
                sql = QString("select count(*) from keep_message where 模具编号 = '%1' and 状态描述 = '定期保养' and 日期 like '%2%';").arg(
                    mold_id).arg(selected);
                if(!select.exec(sql)) {
                ...
                sql = QString("select count(*) from keep_message where 模具编号 = '%1' and 状态描述 = '大保养' and 日期 like '%2%';").arg(
                    mold_id).arg(selected);
                if(!select.exec(sql)) {
                    QMessageBox::warning(this, "导出模具保养盘点信息",
                        "大保养数量查询失败：\n" + select.lastError().text(), QMessageBox::Cancel);
                    ...
                    if(select.next()) {
                        xlsx.write(row, 4, select.value(0).toString(), Song12C);
                    }
                }
                row++;
            }
            fileName = QFileDialog::getSaveFileName(this, "导出模具保养盘点信息",
                    config.value("DOCUMENTCONFIG/DocumentPath").toString() + selected + "成型模具保养盘点记录表",
                    "Excel Files (*.xlsx)");
            file_temp = fileName;
            ...
            if(!xlsx.saveAs(fileName)) {
                QMessageBox::warning(this, "导出模具保养盘点信息", "模具保养盘点记录表创建失败",
                    QMessageBox::Cancel);
            }
        } else {
            QMessageBox::warning(this, "导出模具保养盘点信息",
...
void SumoWindow::on_commandLinkButton_clicked() {
retry:
    bool ok;
...
void SumoWindow::on_KeepButton_clicked() {
    if(id_value.isEmpty()) {
    ...
    QPushButton *way_button3 = way_choice.addButton("转单换线", QMessageBox::ActionRole);
    ...
    if(way_choice.clickedButton() == way_button1) {
        sumoKeep = new SumoKeep(id_value, 1, this);
        connect(sumoKeep, SIGNAL(mysignal()), this, SLOT(refresh_data()));
        sumoKeep->setWindowTitle("异常维修");
        m_windowMap.insert(tr("异常维修"), sumoKeep);
        sumoKeep->show();
    }
    if(way_choice.clickedButton() == way_button2) {
        sumoKeep = new SumoKeep(id_value, 2, this);
        connect(sumoKeep, SIGNAL(mysignal()), this, SLOT(refresh_data()));
        sumoKeep->setWindowTitle("关单保养");
        m_windowMap.insert(tr("关单保养"), sumoKeep);
        sumoKeep->show();
    }
    if(way_choice.clickedButton() == way_button3) {
        sumoKeep = new SumoKeep(id_value, 3, this);
        connect(sumoKeep, SIGNAL(mysignal()), this, SLOT(refresh_data()));
        sumoKeep->setWindowTitle("转单换线");
        m_windowMap.insert(tr("转单换线"), sumoKeep);
        sumoKeep->show();
    }
    if(way_choice.clickedButton() == way_button4) {
        sumoKeep = new SumoKeep(id_value, 4, this);
        connect(sumoKeep, SIGNAL(mysignal()), this, SLOT(refresh_data()));
        sumoKeep->setWindowTitle("定期保养");
        m_windowMap.insert(tr("定期保养"), sumoKeep);
        sumoKeep->show();
    }
    if(way_choice.clickedButton() == way_button5) {
        sumoKeep = new SumoKeep(id_value, 5, this);
        connect(sumoKeep, SIGNAL(mysignal()), this, SLOT(refresh_data()));
        sumoKeep->setWindowTitle("大保养");
        m_windowMap.insert(tr("大保养"), sumoKeep);
        sumoKeep->show();
    }
    if(way_choice.clickedButton() == way_button6) {
        sumoKeep = new SumoKeep(id_value, 6, this);
        connect(sumoKeep, SIGNAL(mysignal()), this, SLOT(refresh_data()));
        sumoKeep->setWindowTitle("呆滞6个月以上");
        m_windowMap.insert(tr("呆滞6个月以上"), sumoKeep);
        sumoKeep->show();
    }
    if(way_choice.clickedButton() == way_button7) {
        sumoKeep = new SumoKeep(id_value, 7);
        connect(sumoKeep, SIGNAL(mysignal()), this, SLOT(refresh_data()));
        sumoKeep->setWindowTitle("其它");
        m_windowMap.insert(tr("其它"), sumoKeep);
        sumoKeep->show();
    }
}

void SumoWindow::on_MessageLabel_clicked() {
    handleMessage = new HandleMessage(curr_auth, curr_name, this);
    handleMessage->setWindowTitle("工单信息管理");
    m_windowMap.insert(tr("工单信息管理"), handleMessage);
    handleMessage->show();
...
void SumoWindow::on_WorkButton_clicked() {
    if(ui->MoldID->text().isEmpty()) {
...
void SumoWindow::sortModelByMaintenanceStatus() {
    if(!standardModel) {
        return;
    }
